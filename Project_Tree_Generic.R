...
category_list <- c('BEST.FILM','BEST.DIRECTOR','BEST.COSTUME','BEST.CINEMATOGRAPHY','BEST.SONG','BEST.MUSIC',
                   ...
                   'BEST.SHORT')
file_category_list <- c('Best_Film','Directing','Costume_Design','Cinematography','Song', 'Score', 
                        'Production_Design', 'MakeUp', 'Editing','Best_Foreign_Film',
                        'Best_Actor','Best_Actress','Best_Sup_Actor','Best_Sup_Actress',
                        'Best_VFX','Best_Doc_Short_Test','Best_Animation','Best_Doc','Best_Animated_Short_Test',
                        'Best_Original_Script','Best_Adapted_Script','Best_Sound_Mixing','Best_Sound_Editing',
                        'Best_Animated_Short_Test')
target_type_list <- c('N','W')
delta_list <- c(10,12,15,18) #The number of immediate past years the model will consider for train
test_range_list <- c(1,2,3,4,5) #The number of years the model will consider for test
...
df_path <- paste('Data Science/Projeto Tree/Tree_',chosen_file_category,'_All_Results.csv',sep="")

if (target_type == 'N') {
  # Arquivo só com Indicados
  df_path <- paste('Data Science/Projeto Tree/Tree_',chosen_file_category,'_Nominees.csv',sep="")
}
...
target_variable <- paste("X...Academy.Awards....Oscar....",chosen_category,".....",target_type,"..",sep="")
